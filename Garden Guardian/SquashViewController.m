...
@interface SquashViewController ()

@end

@implementation SquashViewController

- (void)viewDidLoad {
    self.view.backgroundColor = [UIColor whiteColor];
    [self setTitle:@"Squash"];
    
    UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake(self.view.frame.size.width/2 - 200/2, 70, 200, 200)];
    [imageView setImage:[UIImage imageNamed:@"Squash.jpg"]];
    ...
    for (NSNumber *num in favorites) {
        if ([num integerValue] == VeggieItemSquash) {
            selected = YES;
        }
    }
...
- (void) heartClicked {
    ...
    if (!selected) {
        [favorites addObject:[NSNumber numberWithInt:VeggieItemSquash]];
