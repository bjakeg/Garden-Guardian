//
//  CarnationViewController.m
//  Garden Guardian
//
//  Created by Jake Graham on 3/31/17.
...
@interface CarnationViewController ()

@end

@implementation CarnationViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    
    self.view.backgroundColor = [UIColor whiteColor];
    [self setTitle:@"Carnations"];
    
    UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake(self.view.frame.size.width/2 - 200/2, 70, 200, 200)];
    [imageView setImage:[UIImage imageNamed:@"carnation.jpg"]];
    ...
    plantingInfo.text = @"4 inches deep, 12 inch apart\nSoil > 65F";
    ...
    harvestingInfo.text = @"Prune bush to desired size.\nHarvest flowers 2 weeks after bulb stage";
    ...
    for (NSNumber *num in favorites) {
        if ([num integerValue] == FlowerItemCarnation) {
            selected = YES;
        }
    }
...
- (void) heartClicked {
    ...
    for (NSNumber *num in favorites) {
        if ([num integerValue] == FlowerItemDaisy) {
            saved = num;
            selected = YES;
        }
    }
    if (!selected) {
        [favorites addObject:[NSNumber numberWithInt:FlowerItemDaisy]];
