...
- (void) vectorXSetup
{
    ...
    _vectorX_Steps = (_vectorX_Size) / (_AxisX_partNumber - 1) ;
...
- (void) vectorYSetup
{
    ...
    _vectorY_Steps = (_vectorY_Size) / (_AxisY_partNumber - 1);
            ...
            for (PNScatterChartData *chartData in data) {
                for (NSUInteger i = 0; i < chartData.itemCount; i++) {
                    ...
                    if (!(xValue >= _AxisX_minValue && xValue <= _AxisX_maxValue) || !(yValue >= _AxisY_minValue && yValue <= _AxisY_maxValue)) {
                        NSLog(@"Point not on map. X:%f   Y:%f", xValue, yValue);
                        exit(0);
                    }
...
- (void)drawRect:(CGRect)rect
{
    ...
    if (_showLabel) {
        ...
        for (int i = 0; i < _axisY_labels.count; i++) {
            ...
            CGPoint point = CGPointMake(_AxisX_Margin, temp);
            ...
            CGContextAddLineToPoint(ctx, CGRectGetWidth(rect) - _AxisX_Margin + 5, point.y);
...
- (void)drawCurvedLineFromPoint:(CGPoint)startPoint ToPoint:(CGPoint)endPoint WithLineWidth:(CGFloat)lineWidth AndWithColor:(UIColor *)color {
    // call the same method on a background thread
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        if (self.displayAnimated) {
            [NSThread sleepForTimeInterval:2];
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            // drawing path between two points
            CGPoint midPoint = [PNLineChart midPointBetweenPoint1:CGPointMake(startX, startY) andPoint2:CGPointMake(endX, endY)];
            UIBezierPath *path = [UIBezierPath bezierPath];
            [path moveToPoint:CGPointMake(startX, startY)];
            [path addQuadCurveToPoint:midPoint
                         controlPoint:[PNLineChart controlPointBetweenPoint1:midPoint andPoint2:CGPointMake(startX, startY)]];
            [path addQuadCurveToPoint:CGPointMake(endX, endY)
                         controlPoint:[PNLineChart controlPointBetweenPoint1:midPoint andPoint2:CGPointMake(endX, endY)]];
...
- (void)touchPoint:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    for (PNScatterChartData *chartData in _chartData) {
        for (NSUInteger i = 0; i < chartData.itemCount; i++) {
            CGFloat yValue = chartData.getData(i).y;
            CGFloat xValue = chartData.getData(i).x;
            ...
            CGFloat distance = sqrt(pow(touchPoint.y - yFinilizeValue, 2) + pow(touchPoint.x - xFinilizeValue, 2));
        ...
        if (minIndex != -1) {
            CGFloat yValue = chartData.getData(minIndex).y;
            CGFloat xValue = chartData.getData(minIndex).x;
