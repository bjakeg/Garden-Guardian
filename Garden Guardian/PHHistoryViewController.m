...
@interface PHHistoryViewController () <PNChartDelegate>

@end

@implementation PHHistoryViewController {
...
- (void)viewDidLoad {
    ...
    [self setTitle:@"pH Level"];
    
    [self setupLeftMenuButton];
    
    _titleLabel = [[UILabel alloc] init];
    _titleLabel.font = [UIFont boldSystemFontOfSize:20];
    _titleLabel.text = @"pH Level";
...
- (NSArray *)randomSetOfObjects {
    ...
    for (int i = 0; i < 50; i++) {
        ...
        int mid = arc4random()%20;
        CGFloat next = (float)mid/10.0f;
        [YAr addObject:[NSString stringWithFormat:LabelFormat, 6.5f + next]];
    }
    [array addObject:XAr];
    [array addObject:YAr];
    return array;
...
- (void)addGraph {
    ...
    [_scatterChart setAxisYWithMinimumValue:0 andMaxValue:14 toTicks:5];
    ...
    data01.getData = ^(NSUInteger index) {
        ...
        CGFloat yValue = [yString floatValue];
