...
@interface CucumberViewController ()

@end

@implementation CucumberViewController

- (void)viewDidLoad {
    [super viewDidLoad];

    self.view.backgroundColor = [UIColor whiteColor];
    [self setTitle:@"Cucumber"];
    
    UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake(self.view.frame.size.width/2 - 200/2, 70, 200, 200)];
    [imageView setImage:[UIImage imageNamed:@"cucumber.jpg"]];
    ...
    harvestingInfo.text = @"Prune to size of rows\nHarvest when slightly longer than hand\nHarvest when length of hand for pickling";
    harvestingInfo.numberOfLines = 3;
    ...
    for (NSNumber *num in favorites) {
        if ([num integerValue] == VeggieItemCucumber) {
            selected = YES;
        }
    }
...
- (void) heartClicked {
    ...
    if (!selected) {
        [favorites addObject:[NSNumber numberWithInt:VeggieItemCucumber]];
